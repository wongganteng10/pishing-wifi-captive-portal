...
sudo ifconfig wlan0mon 192.169.1.1 netmask 255.255.255.0 up
...
sudo iptables -t nat -A PREROUTING -i wlan0mon -p tcp --dport 80 -j REDIRECT --to-port 80
sudo iptables -t nat -A PREROUTING -i wlan0mon -p tcp --dport 443 -j REDIRECT --to-port 80

sudo iptables -A INPUT -i wlan0mon -p udp --dport 53 -j ACCEPT
sudo iptables -A INPUT -i wlan0mon -p tcp --dport 80 -j ACCEPT
sudo iptables -A INPUT -i wlan0mon -p tcp --dport 443 -j ACCEPT
EOL
...
# ===================================
sudo tee /wifi/wifi_free-wifi.sh > /dev/null <<'EOL'
#!/bin/bash
...
# Pastikan wlan0mon ada
if ! ip link show wlan0mon >/dev/null 2>&1; then
    echo "Interface wlan0mon tidak ditemukan!"
    exit 1
fi
...
run_service "sudo dhcpd -d -cf /wifi/conf/wifi-dhcpd.conf wlan0mon" "DHCPD" $POS_G2_X $POS_G2_Y
...
EOL
sudo chmod +x /wifi/wifi_free-wifi.sh
#echo "/wifi/wifi_free-wifi.sh berhasil dibuat."
if [ $? -eq 0 ]; then
    echo "✅ File : /wifi/wifi_free-wifi.sh berhasil dibuat."
else
    echo "❌ ERROR: /wifi/wifi_free-wifi.sh gagal dibuat..."
    # Opsional: Tampilkan pesan error lebih detail jika Anda punya logging khusus
fi
#echo ""
...
# ========================================
sudo cp -f /wifi/conf/*.conf /wifi/conf/backup-conf/
sudo cp -f /wifi/conf/backup-conf/dhcpd.conf /etc/dhcp/wifi-dhcpd.conf
...
# ========================================
sudo chown -R $USER:$USER /wifi
echo "✅ File /wifi berhasil diubah kepemilikan nya."
...
echo "💻 sudo bash /wifi/wifi_free-wifi.sh"
echo "✨ Atau..."
echo "💻 sudo ./wifi/wifi_free-wifi.sh"
#echo ""
