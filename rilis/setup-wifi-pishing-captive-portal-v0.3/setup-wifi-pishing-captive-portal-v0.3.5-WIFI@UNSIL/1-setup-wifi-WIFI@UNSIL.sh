...
# ===================================
sudo tee /tmp/wifi-WIFI@UNSIL/wifi_WIFI@UNSIL.sh > /dev/null <<'EOL'
#!/bin/bash
...
EOL
sudo chmod +x /tmp/wifi-WIFI@UNSIL/wifi_WIFI@UNSIL.sh
#echo "/tmp/wifi-WIFI@UNSIL/wifi_WIFI@UNSIL.sh berhasil dibuat."
if [ $? -eq 0 ]; then
    echo "✅ File : /tmp/wifi-WIFI@UNSIL/wifi_WIFI@UNSIL.sh berhasil dibuat."
else
    echo "❌ ERROR: /tmp/wifi-WIFI@UNSIL/wifi_WIFI@UNSIL.sh gagal dibuat..."
    # Opsional: Tampilkan pesan error lebih detail jika Anda punya logging khusus
fi
#echo ""
...
echo "💻 sudo bash /tmp/wifi-WIFI@UNSIL/wifi_WIFI@UNSIL.sh"
echo "✨ Atau..."
echo "💻 sudo ./tmp/wifi-WIFI@UNSIL/wifi_WIFI@UNSIL.sh"
#echo ""
