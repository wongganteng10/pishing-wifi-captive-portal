...
# ===================================
if [ ! -d "/free-wifi" ]; then
    echo "✅ Membuat folder : /free-wifi dan subfolder..."
    sudo mkdir -p /free-wifi/www /free-wifi/conf /free-wifi/conf/backup-conf /free-wifi/log /free-wifi/sh /free-wifi/portal/ || { echo "❌ Gagal membuat folder!"; exit 1; }
fi

# ===================================
# Buat hostapd.conf
# ===================================
sudo tee /free-wifi/conf/hostapd.conf > /dev/null <<EOL
interface=wlan0
driver=nl80211
ssid=free-wifi
...
hw_mode=g
EOL
#echo "File /free-wifi/conf/hostapd.conf berhasil dibuat."
if [ $? -eq 0 ]; then
    echo "✅ File : /free-wifi/conf/hostapd.conf berhasil dibuat."
else
    echo "❌ ERROR: File /free-wifi/conf/hostapd.conf gagal dibuat..."
...
# Buat dnsmasq.conf
# ===================================
sudo tee /free-wifi/conf/dnsmasq.conf > /dev/null <<EOL
interface=wlan0
address=/#/192.169.1.1
port=53
bind-dynamic
...
no-hosts
EOL
#echo "/free-wifi/conf/dnsmasq.conf berhasil dibuat."
if [ $? -eq 0 ]; then
    echo "✅ File : /free-wifi/conf/dnsmasq.conf berhasil dibuat."
else
    echo "❌ ERROR: File /free-wifi/conf/dnsmasq.conf gagal dibuat..."
    # Opsional: Tampilkan pesan error lebih detail jika Anda punya logging khusus
fi
#echo ""
...
# Buat dhcpd.conf
# ===================================
sudo tee /free-wifi/conf/dhcpd.conf > /dev/null <<EOL
authoritative;
...
EOL
#echo "File /free-wifi/conf/dhcpd.conf berhasil dibuat."
if [ $? -eq 0 ]; then
    echo "✅ File : /free-wifi/conf/dhcpd.conf berhasil dibuat."
else
    echo "❌ ERROR: File /free-wifi/conf/dhcpd.conf gagal dibuat..."
    # Opsional: Tampilkan pesan error lebih detail jika Anda punya logging khusus
fi
#echo ""
...
# Buat lighttpd.conf
# ===================================
sudo tee /free-wifi/conf/lighttpd.conf > /dev/null <<EOL
...
EOL
#echo "File /free-wifi/conf/lighttpd.conf berhasil dibuat."
if [ $? -eq 0 ]; then
    echo "✅ File : /free-wifi/conf/lighttpd.conf berhasil dibuat."
else
    echo "❌ ERROR: File /free-wifi/conf/lighttpd.conf gagal dibuat..."
    # Opsional: Tampilkan pesan error lebih detail jika Anda punya logging khusus
fi
#echo ""
...
# Buat start.sh
# ===================================
sudo tee /free-wifi/sh/start.sh > /dev/null <<'EOL'
#!/bin/bash
...
EOL
sudo chmod +x /free-wifi/sh/start.sh
#echo "/free-wifi/sh/start.sh berhasil dibuat."
if [ $? -eq 0 ]; then
    echo "✅ File : /free-wifi/sh/start.sh berhasil dibuat."
else
    echo "❌ ERROR: /free-wifi/sh/start.sh gagal dibuat..."
    # Opsional: Tampilkan pesan error lebih detail jika Anda punya logging khusus
fi
#echo ""
...
# Buat stop.sh
# ===================================
sudo tee /free-wifi/sh/stop.sh > /dev/null <<'EOL'
#!/bin/bash
sudo pkill lighttpd
sudo pkill dnsmasq
sudo pkill hostapd
...
EOL
sudo chmod +x /free-wifi/sh/stop.sh
#echo "/free-wifi/sh/stop.sh berhasil dibuat."
if [ $? -eq 0 ]; then
    echo "✅ File : /free-wifi/sh/stop.sh berhasil dibuat."
else
    echo "❌ ERROR: /free-wifi/sh/stop.sh gagal dibuat..."
    # Opsional: Tampilkan pesan error lebih detail jika Anda punya logging khusus
fi
#echo ""
...
# ===================================
# Buat ubuntu-free-wifi.sh
# ===================================
sudo tee /free-wifi/ubuntu-free-wifi.sh > /dev/null <<'EOL'
#!/bin/bash

X_MARGIN=10
Y_MARGIN=20
START_SCRIPT="/free-wifi/sh/start.sh"
STOP_SCRIPT="/free-wifi/sh/stop.sh"
SERVICE_PIDS=()
...
# run_service "sudo hostapd /free-wifi/conf/hostapd.conf" "HOSTAPD" $POS_G1_X $POS_G1_Y
# run_service "sudo dhcpd -d -cf /free-wifi/conf/dhcpd.conf wlan0" "DHCPD" $POS_G2_X $POS_G2_Y
# run_service "sudo dnsmasq -C /free-wifi/conf/dnsmasq.conf" "DNSMASQ" $POS_G3_X $POS_G3_Y
# run_service "sudo lighttpd -D -f /free-wifi/conf/lighttpd.conf" "LIGHTTPD" $POS_G4_X $POS_G4_Y

sudo hostapd /free-wifi/conf/hostapd.conf &
sudo dnsmasq -C /free-wifi/conf/dnsmasq.conf &
sudo lighttpd -D -f /free-wifi/conf/lighttpd.conf &
...
EOL
sudo chmod +x /free-wifi/ubuntu-free-wifi.sh
#echo "/free-wifi/ubuntu-free-wifi.sh berhasil dibuat."
if [ $? -eq 0 ]; then
    echo "✅ File : /free-wifi/ubuntu-free-wifi.sh berhasil dibuat."
else
    echo "❌ ERROR: /free-wifi/ubuntu-free-wifi.sh gagal dibuat..."
    # Opsional: Tampilkan pesan error lebih detail jika Anda punya logging khusus
fi
#echo ""
...
# ========================================
sudo cp -f /free-wifi/conf/*.conf /free-wifi/conf/backup-conf/
sudo cp -f /free-wifi/conf/backup-conf/dhcpd.conf /etc/dhcp/wifi-dhcpd.conf
...
if [ $? -eq 0 ]; then
    ...
    echo "✅ folder file conf : /free-wifi/conf/backup-conf/"
else
...
# ========================================
sudo cp -rf ./www/* /free-wifi/www/
sudo dos2unix /free-wifi/www/*.htm* 
sudo dos2unix /free-wifi/www/css-js/* 

# Cek exit status dari perintah 'sudo cp' yang baru saja dijalankan
if [ $? -eq 0 ]; then
    echo "✅ Salinan file /free-wifi/www/ berhasil!"
else
    echo "❌ ERROR: Salinan file /free-wifi/www/ gagal."
    # Opsional: Tampilkan pesan error lebih detail jika Anda punya logging khusus
fi
...
# ========================================
sudo chown -R $USER:$USER /free-wifi
echo "✅ File /free-wifi berhasil diubah kepemilikan nya."
...
echo "💻 sudo bash /free-wifi/ubuntu-free-wifi.sh"
echo "✨ Atau..."
# echo "💻 sudo ./free-wifi/ubuntu-free-wifi.sh"
#echo ""
